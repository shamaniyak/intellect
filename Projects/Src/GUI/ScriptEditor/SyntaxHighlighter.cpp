	...
	m_highlighter(NULL)
{
	...
	m_builtInColor = Qt::darkMagenta;
	m_markerColor = Qt::black;
	m_itemColor = Qt::black;
	m_commentColor = Qt::darkGray;
	m_keywordColor = Qt::blue;
	m_propertyColor = Qt::darkMagenta;
	m_stringColor = Qt::darkGreen;
	m_numberColor = Qt::darkMagenta;
}

void SyntaxHighlighter::setDocument(QObject *textArea)
{
	QQuickTextDocument *quickTextDocument = qobject_cast<QQuickTextDocument*>(textArea);
	QTextDocument *document = quickTextDocument ? quickTextDocument->textDocument() : nullptr;
	if(!document)
		document = qobject_cast<QTextDocument*>(textArea);
	if(!document)
		return;
